...
(async function () {
  ...
  function splitList_(str) { return String(str || '').split(',').map(function (s) { return s.trim(); }).filter(Boolean); }
  ...
  function renderHomework() {
    ...
    document.getElementById('hwTableBody').innerHTML = rows.map(function (h) {
      ...
      return '<tr><td style="font-weight:600;">' + BYD.escapeHtml(h.Title) + '</td>' +
        ...
        '<td class="text-sm"><button class="link-btn" data-view-completions="' + h.ID + '">' + done + ' / ' + total + '</button></td>' +
    ...
    document.querySelectorAll('[data-view-completions]').forEach(function (b) {
      b.addEventListener('click', function () { openHwCompletionsModal(homework.find(function (h) { return h.ID === b.dataset.viewCompletions; })); });
    });
  ...
  function openHwCompletionsModal(hw) {
    const roster = students.filter(function (s) { return s.group === hw.Group; }).sort(function (a, b) { return a.name.localeCompare(b.name); });
    const doneEmails = homeworkCompletions.filter(function (c) { return c.HomeworkID === hw.ID; }).map(function (c) { return c.StudentEmail; });
    const rows = roster.map(function (s) {
      const done = doneEmails.indexOf(s.email) > -1;
      return '<tr><td>' + BYD.escapeHtml(s.name) + '</td><td class="font-mono text-sm">' + BYD.escapeHtml(s.email) + '</td>' +
        '<td>' + (done ? '<span class="badge badge-present">Done</span>' : '<span class="badge badge-unmarked">Not yet</span>') + '</td></tr>';
    }).join('') || '<tr><td colspan="3" class="text-muted">No students in this group.</td></tr>';
    BYD.openModal(
      '<div class="modal-head"><h2>' + BYD.escapeHtml(hw.Title) + '</h2><button class="modal-close" onclick="BYD.closeModal()">\u00d7</button></div>' +
      '<p class="text-sm text-muted">' + doneEmails.length + ' of ' + roster.length + ' in ' + BYD.escapeHtml(hw.Group) + ' have marked this done.</p>' +
      '<div class="table-wrap"><table><thead><tr><th>Student</th><th>Email</th><th>Status</th></tr></thead><tbody>' + rows + '</tbody></table></div>' +
  ...
  function renderSchedulePreview() {
    ...
    document.getElementById('schPreviewTableBody').innerHTML = rows.map(function (s) {
      ...
      return '<tr' + (isToday ? ' style="background:var(--ink-tint);"' : '') + '><td class="font-mono">' + BYD.fmtDate(s.Date) + '</td>' +
        ...
        '<td class="text-muted">' + BYD.escapeHtml(s.Notes) + '</td>' +
        '<td class="text-muted">' + BYD.escapeHtml(s.Coaches) + '</td></tr>';
    }).join('');
  }

  // ----------------------------------------------------------- rounds
  function rfdsForPairing(pairingId) { return rfds.filter(function (r) { return r.PairingID === pairingId; }); }
  ...
  function renderRounds() {
    ...
    document.getElementById('roundsList').innerHTML = rows.map(function (r) {
      ...
      const pairingRows = rp.map(function (p) {
        const panelEmails = splitList_(p.JudgeEmail);
        const panelNames = splitList_(p.Judge);
        const myRfd = rfdsForPairing(p.ID).find(function (r) { return r.JudgeEmail === user.email; });
        const isOnPanel = panelEmails.indexOf(user.email) > -1;
        const canSubmitRfd = !panelEmails.length || isOnPanel || user.role === 'admin';

        const judgeChips = panelNames.map(function (name, i) {
          const email = panelEmails[i] || '';
          const isMe = email === user.email;
          return '<span class="judge-chip">' + BYD.escapeHtml(name) + (isMe ? ' (you)' : '') +
            (isMe ? '<button type="button" data-unclaim-judge="' + p.ID + '" title="Leave panel">\u00d7</button>' : '') + '</span>';
        }).join('');
        const judgeCell = '<div class="judge-panel-list">' + judgeChips +
          (isOnPanel ? '' : '<button type="button" class="btn btn-ghost btn-sm" data-claim-judge="' + p.ID + '">Join judging panel</button>') + '</div>';

        const rfdCell = '<button class="btn btn-ghost btn-sm" data-rfd="' + p.ID + '"' + (canSubmitRfd ? '' : ' disabled title="Only a judge on this round\u2019s panel can submit"') + '>' + (myRfd ? 'Edit my RFD' : 'RFD') + '</button>';
        ...
        const allRfds = rfdsForPairing(p.ID);
        const rfdRow = allRfds.length
          ? '<tr><td colspan="8">' + allRfds.map(function (rfd) {
              return '<div class="rfd-box"><div class="rfd-winner">Winner: ' + BYD.escapeHtml(rfd.Winner === 'Side1' ? p.Side1 : p.Side2) + '</div>' +
                (rfd.Feedback ? '<div class="rfd-feedback">' + BYD.escapeHtml(rfd.Feedback) + '</div>' : '') +
                '<div class="text-sm text-muted mt-8">Judged by ' + BYD.escapeHtml(rfd.JudgeName) + '</div></div>';
            }).join('') + '</td></tr>'
          : '';
        ...
        return '<tr><td><span class="side-tag side-tag-1">' + BYD.escapeHtml(p.Side1Label || 'Aff') + '</span></td>' +
          ...
          '<td class="font-mono text-sm">' + BYD.escapeHtml(p.Room) + '</td>' +
          '<td>' + judgeCell + '</td>' +
      ...
      return '<div class="card">' +
        ...
        '<div class="table-wrap"><table><thead><tr><th></th><th>Side 1</th><th></th><th>Side 2</th><th>Room</th><th>Judging panel</th><th>RFD</th><th></th></tr></thead>' +
    ...
    document.querySelectorAll('[data-claim-judge]').forEach(function (b) {
      b.addEventListener('click', async function () {
        try {
          ...
          pairings = await BYD.call('getPairings', {});
          renderRounds();
          BYD.toast("You've joined the judging panel.", 'success');
        } catch (err) { BYD.toast(BYD.errorMessage(err), 'error'); }
      });
    });
    document.querySelectorAll('[data-unclaim-judge]').forEach(function (b) {
      b.addEventListener('click', async function () {
        try {
          await BYD.call('unclaimJudge', { pairingId: b.dataset.unclaimJudge });
          pairings = await BYD.call('getPairings', {});
          renderRounds();
          BYD.toast('Left the judging panel.', 'success');
        } catch (err) { BYD.toast(BYD.errorMessage(err), 'error'); }
      });
    });
  ...
  // selects auto-fill the side text + emails. Room is auto-assigned by the
  // backend (reassignRoomsForDate_) right after saving, so it's read-only
  // on Add and only worth hand-editing afterward if you really need to
  // override the auto sequence. Judge/JudgeEmail are a panel (comma
  // list) - "Add my name" appends to it rather than overwriting, so it
  // never wipes out judges already on the panel.
  function openPairingModal(roundId, existing) {
    ...
    BYD.openModal(
      ...
      '<hr class="divider">' +
      '<div class="field"><label for="pRoom">Room</label><input type="text" id="pRoom"' + (existing ? '' : ' disabled placeholder="Assigned automatically once saved"') + ' value="' + BYD.escapeHtml(existing ? existing.Room : '') + '"></div>' +
      '<div class="field mb-0"><label for="pJudge">Judging panel</label><div class="flex gap-8"><input type="text" id="pJudge" style="flex:1;" placeholder="Comma-separated names, optional" value="' + BYD.escapeHtml(existing ? existing.Judge : '') + '"><button type="button" class="btn btn-ghost btn-sm" id="pClaimJudgeBtn">Add my name</button></div></div>' +
    ...
    document.getElementById('pClaimJudgeBtn').addEventListener('click', function () {
      const nameField = document.getElementById('pJudge');
      const emailField = document.getElementById('pJudgeEmail');
      const names = splitList_(nameField.value);
      const emails = splitList_(emailField.value);
  ...
  function openRfdModal(pairing) {
    const existing = rfdsForPairing(pairing.ID).find(function (r) { return r.JudgeEmail === user.email; });
    BYD.openModal(
      '<div class="modal-head"><h2>RFD</h2><button class="modal-close" onclick="BYD.closeModal()">\u00d7</button></div>' +
      '<p class="text-sm text-muted">' + BYD.escapeHtml(pairing.Side1) + ' (' + BYD.escapeHtml(pairing.Side1Label || 'Aff') + ') vs. ' + BYD.escapeHtml(pairing.Side2) + ' (' + BYD.escapeHtml(pairing.Side2Label || 'Neg') + ')' + (existing ? ' \u2014 this is your own RFD; other judges on the panel submit theirs separately.' : '') + '</p>' +
      '<form id="rfdForm">' +
  ...
  // (date, student) - see upsertAttendance_ in Data.gs. Statuses are only
  // held in memory (`pendingAttendance`) as a coach clicks through the
  // roster - nothing hits the network, and nothing re-fetches/re-renders
  // the whole table, until "Submit attendance" sends everything in one
  // request (setAttendanceBulk). That's what fixes the old one-click-at-a-
  // time feel of the whole table flashing/reloading on every click.
  // Controls are disabled entirely (see attNoPractice below) unless the
  // selected date has a real Schedule entry for the selected group.
  let attRoster = [];
  let pendingAttendance = {}; // email -> status, unsaved edits for the current group/date
  let savedAttendance = {};   // email -> status, last known saved state (to detect "dirty")
  ...
  function initAttendanceControls() {
    ...
    document.getElementById('attDateSelect').onchange = function () { loadAttendanceTable(); renderAttendanceMatrix(); };
    loadAttendanceTable();
  ...
  async function loadAttendanceTable() {
    ...
    if (!group || !date) { body.innerHTML = '<tr><td colspan="2" class="text-muted">Pick a group and date.</td></tr>'; return; }
    ...
    document.getElementById('attSubmitBar').classList.toggle('hidden', !isRealPractice);
    if (!isRealPractice) { body.innerHTML = ''; return; }

    body.innerHTML = '<tr><td colspan="2"><div class="skeleton" style="height:20px;"></div></td></tr>';
    attRoster = students.filter(function (s) { return s.group === group; }).sort(function (a, b) { return a.name.localeCompare(b.name); });
    const records = await BYD.call('getAttendance', { group: group, date: date });
    savedAttendance = {};
    attRoster.forEach(function (s) {
      const rec = records.find(function (r) { return r.StudentEmail === s.email; });
      savedAttendance[s.email] = rec ? rec.Status : 'Unmarked';
    });
    pendingAttendance = Object.assign({}, savedAttendance);
    renderAttendanceRows();
  }

  function renderAttendanceRows() {
    const body = document.getElementById('attTableBody');
    if (!attRoster.length) { body.innerHTML = '<tr><td colspan="2" class="text-muted">No students in this group yet.</td></tr>'; updateAttDirtyState(); return; }
    body.innerHTML = attRoster.map(function (s) {
      const status = pendingAttendance[s.email] || 'Unmarked';
      const isDirty = status !== savedAttendance[s.email];
      return '<tr><td>' + (isDirty ? '<span class="att-dirty-dot" title="Unsaved change"></span>' : '') + BYD.escapeHtml(s.name) +
        '<div class="text-sm text-muted">' + BYD.escapeHtml(s.email) + '</div></td>' +
        '<td class="table-actions">' +
        ['Present', 'Absent', 'Excused'].map(function (st) {
          const active = st === status;
          return '<button type="button" class="btn ' + (active ? 'btn-primary' : 'btn-ghost') + ' btn-sm" data-set-att="' + BYD.escapeHtml(s.email) + '" data-status="' + st + '">' + st + '</button>';
        }).join('') +
        (status !== 'Unmarked' ? '<button type="button" class="btn btn-ghost btn-sm" data-set-att="' + BYD.escapeHtml(s.email) + '" data-status="Unmarked">Clear</button>' : '') +
        '</td></tr>';
    }).join('');
    body.querySelectorAll('[data-set-att]').forEach(function (btn) {
      btn.addEventListener('click', function () {
        pendingAttendance[btn.dataset.setAtt] = btn.dataset.status;
        renderAttendanceRows(); // instant, local re-render - no network call, no flicker
      });
    });
    updateAttDirtyState();
  }

  function updateAttDirtyState() {
    const dirtyCount = Object.keys(pendingAttendance).filter(function (email) { return pendingAttendance[email] !== savedAttendance[email]; }).length;
    const submitBtn = document.getElementById('attSubmitBtn');
    const label = document.getElementById('attDirtyLabel');
    submitBtn.disabled = dirtyCount === 0;
    label.textContent = dirtyCount === 0
      ? 'Pick statuses above, then submit \u2014 nothing saves until you click Submit.'
      : dirtyCount + ' unsaved change' + (dirtyCount === 1 ? '' : 's') + '.';
  }

  document.getElementById('attSubmitBtn').addEventListener('click', async function () {
    const date = document.getElementById('attDateSelect').value;
    const changed = Object.keys(pendingAttendance)
      .filter(function (email) { return pendingAttendance[email] !== savedAttendance[email]; })
      .map(function (email) { return { studentEmail: email, status: pendingAttendance[email] }; });
    if (!changed.length) return;
    const btn = this;
    btn.disabled = true; btn.textContent = 'Submitting\u2026';
    try {
      await BYD.call('setAttendanceBulk', { date: date, updates: changed });
      BYD.toast(changed.length + ' attendance record' + (changed.length === 1 ? '' : 's') + ' saved.', 'success');
  ...
  function initAutoPairControls() {
    const groupSelect = document.getElementById('autoPairGroupSelect');
    groupSelect.innerHTML = nonUnassignedGroups().map(function (g) {
      return '<option value="' + BYD.escapeHtml(g.GroupName) + '">' + BYD.escapeHtml(g.GroupName) + '</option>';
    }).join('');
    if (!document.getElementById('autoPairDateSelect').value) document.getElementById('autoPairDateSelect').value = BYD.todayIso();
  }

  document.getElementById('autoPairBtn').addEventListener('click', async function () {
    const group = document.getElementById('autoPairGroupSelect').value;
    const date = document.getElementById('autoPairDateSelect').value;
    const btn = this;
    btn.disabled = true;
    try {
    ...
    document.querySelectorAll('[data-del-group]').forEach(function (b) {
      b.addEventListener('click', function () {
        confirmDelete('Delete group "' + b.dataset.delGroup + '"? Students must be moved out first.', async function () {
          ...
          renderRoster(); renderHomework(); renderSchedulePreview(); renderRounds(); initAttendanceControls(); initAutoPairControls();
  ...
  // The Schedule and Attendance tabs both depend on the in-memory
  // `schedule` array, which is only loaded once at page open - but the
  // whole point of the Sheet-driven schedule is that Evan can add a row
  // and switch straight back to the app, so a stale copy would make a
  // brand-new practice look like it doesn't exist yet. Re-fetch it fresh
  // every time either tab is opened, not just on an explicit "Refresh"
  // click or a full page reload.
  document.addEventListener('byd:view', async function (e) {
    const view = e.detail && e.detail.view;
    if (view !== 'schedule' && view !== 'attendance') return;
    try {
      schedule = await BYD.call('getSchedule', {});
      if (view === 'schedule') { renderSchedulePreview(); renderHome(); }
      if (view === 'attendance') { loadAttendanceTable(); renderAttendanceMatrix(); }
    } catch (err) { /* non-fatal - whatever was already loaded stays visible */ }
  });
