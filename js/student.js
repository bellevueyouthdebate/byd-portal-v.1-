...
(async function () {
  ...
  function renderRounds() {
    ...
    wrap.innerHTML = sorted.map(function (r) {
      ...
      const rowsHtml = rp.map(function (p) {
        ...
        const pairingRfds = rfdsForPairing(p.ID);
        let rfdHtml = '';
        if (mine && pairingRfds.length) {
          rfdHtml = '<tr' + (mine ? ' style="background:var(--ink-tint);"' : '') + '><td colspan="7">' +
            pairingRfds.map(function (rfd) {
              const winnerName = rfd.Winner === 'Side1' ? p.Side1 : p.Side2;
              return '<div class="rfd-box"><div class="rfd-winner">Winner: ' + BYD.escapeHtml(winnerName) + '</div>' +
  ...
  // Same reasoning as coach.js: `schedule` is only fetched once at page
  // load, but a coach can add a practice via the Sheet at any time while
  // this page is already open. Re-fetch it whenever the Schedule or
  // Attendance tab is opened so a newly-added practice (or today suddenly
  // becoming a practice day) doesn't require a full page reload to show up.
  document.addEventListener('byd:view', async function (e) {
    ...
    try {
      [schedule, attendance] = await Promise.all([BYD.call('getSchedule', {}), BYD.call('getAttendance', {})]);
      if (view === 'schedule') { renderSchedule(); renderHome(); }
