...
(async function () {
  ...
  // Permanent delete is irreversible, so it gets a stronger gate than the
  // usual confirm dialog - the admin has to type the account's email
  // before the button even enables.
  function confirmHardDelete(email, kindLabel, onConfirm) {
    BYD.openModal(
      '<div class="modal-head"><h2>Permanently delete this ' + kindLabel + '?</h2><button class="modal-close" onclick="BYD.closeModal()">\u00d7</button></div>' +
      '<p class="text-sm text-muted">This cannot be undone. It fully removes <b>' + BYD.escapeHtml(email) + '</b> \u2014 their login, and their attendance / homework-completion / season-enrollment history. (This is different from "Remove"/"Deactivate", which just locks them out but keeps their history and can be undone.)</p>' +
      '<div class="field mb-0"><label for="hardDeleteConfirmInput">Type their email to confirm</label><input type="text" id="hardDeleteConfirmInput" placeholder="' + BYD.escapeHtml(email) + '" autocomplete="off"></div>' +
      '<div class="modal-actions"><button type="button" class="btn btn-ghost" id="hardDeleteCancel">Cancel</button>' +
      '<button type="button" class="btn btn-danger" id="hardDeleteOk" disabled>Delete permanently</button></div>'
    );
    const input = document.getElementById('hardDeleteConfirmInput');
    const okBtn = document.getElementById('hardDeleteOk');
    input.addEventListener('input', function () { okBtn.disabled = input.value.trim().toLowerCase() !== email.toLowerCase(); });
    document.getElementById('hardDeleteCancel').addEventListener('click', function () { BYD.closeModal(); });
    okBtn.addEventListener('click', async function () {
      okBtn.disabled = true; okBtn.textContent = 'Deleting\u2026';
      try { await onConfirm(); BYD.closeModal(); } catch (err) { BYD.toast(BYD.errorMessage(err), 'error'); okBtn.disabled = false; okBtn.textContent = 'Delete permanently'; }
    });
  }
  ...
  function renderStaff() {
    document.getElementById('staffTableBody').innerHTML = staff.slice().sort(function (a, b) { return a.name.localeCompare(b.name); }).map(function (s) {
      ...
      return '<tr><td>' + BYD.escapeHtml(s.name) + (isSelf ? ' <span class="text-muted text-sm">(you)</span>' : '') + '</td>' +
        ...
        (isSelf ? '' :
          ...
          '<button class="btn btn-danger btn-sm" data-hard-delete-staff="' + BYD.escapeHtml(s.email) + '">Delete permanently</button>') +
        '</td></tr>';
    }).join('');
    ...
    document.querySelectorAll('[data-hard-delete-staff]').forEach(function (btn) {
      btn.addEventListener('click', function () {
        const email = btn.dataset.hardDeleteStaff;
        confirmHardDelete(email, 'coach/admin account', async function () {
          await BYD.call('deleteUserPermanently', { email: email });
          BYD.toast('Account permanently deleted.', 'success');
          staff = await BYD.call('getStaff', {});
          renderStaff(); renderHome();
        });
      });
  ...
  // ------------------------------------------------------------ students
  // Admin-only "remove a student" page, same table/confirm/reset-password
  // pattern as Staff accounts above (handleSetUserActive_/resetUserPassword
  // already allow admins to act on any student - this just gives it its
  // own dedicated view instead of only living inside the Coach Console).
  function renderStudents() {
    const chipsEl = document.getElementById('studentGroupFilter');
    const groupNames = ['All'].concat(groups.map(function (g) { return g.GroupName; }));
    chipsEl.innerHTML = groupNames.map(function (g) {
      return '<button type="button" class="chip' + (g === studentGroupFilter ? ' active' : '') + '" data-g="' + BYD.escapeHtml(g) + '">' + BYD.escapeHtml(g) + '</button>';
    }).join('');
    chipsEl.querySelectorAll('.chip').forEach(function (chip) {
      chip.addEventListener('click', function () { studentGroupFilter = chip.dataset.g; renderStudents(); });
    });

    const rows = students.filter(function (s) { return studentGroupFilter === 'All' || s.group === studentGroupFilter; })
      .slice().sort(function (a, b) { return a.name.localeCompare(b.name); });

    document.getElementById('adminStudentsTableBody').innerHTML = rows.map(function (s) {
      return '<tr><td>' + BYD.escapeHtml(s.name) + '</td>' +
        '<td class="font-mono text-sm">' + BYD.escapeHtml(s.email) + '</td>' +
        '<td><span class="badge badge-role">' + BYD.escapeHtml(s.group) + '</span></td>' +
        '<td>' + (s.active ? '<span class="badge badge-present">Active</span>' : '<span class="badge badge-absent">Inactive</span>') + '</td>' +
        '<td class="table-actions">' +
        '<button class="btn btn-ghost btn-sm" data-student-reset-pw="' + BYD.escapeHtml(s.email) + '">Reset password</button>' +
        '<button class="btn btn-danger btn-sm" data-student-toggle-active="' + BYD.escapeHtml(s.email) + '" data-active="' + s.active + '">' + (s.active ? 'Remove' : 'Restore') + '</button>' +
        '<button class="btn btn-danger btn-sm" data-hard-delete-student="' + BYD.escapeHtml(s.email) + '">Delete permanently</button>' +
        '</td></tr>';
    }).join('') || '<tr><td colspan="5" class="text-muted">No students in this group.</td></tr>';

    document.querySelectorAll('[data-student-reset-pw]').forEach(function (btn) {
      btn.addEventListener('click', async function () {
        try {
          const data = await BYD.call('resetUserPassword', { email: btn.dataset.studentResetPw });
          BYD.openModal(
            '<div class="modal-head"><h2>Password reset</h2><button class="modal-close" onclick="BYD.closeModal()">\u00d7</button></div>' +
            '<p class="text-sm">Give this temporary password to <b>' + BYD.escapeHtml(btn.dataset.studentResetPw) + '</b>. They should change it after logging in.</p>' +
            '<p class="font-mono" style="font-size:18px; background:var(--paper); padding:10px 14px; border-radius:6px;">' + BYD.escapeHtml(data.tempPassword) + '</p>' +
            '<div class="modal-actions"><button type="button" class="btn btn-primary" onclick="BYD.closeModal()">Done</button></div>'
          );
        } catch (err) { BYD.toast(BYD.errorMessage(err), 'error'); }
    ...
    document.querySelectorAll('[data-student-toggle-active]').forEach(function (btn) {
      btn.addEventListener('click', function () {
        const goingActive = btn.dataset.active !== 'true';
        const action = async function () {
          await BYD.call('setUserActive', { email: btn.dataset.studentToggleActive, active: goingActive });
          BYD.toast(goingActive ? 'Student restored.' : 'Student removed.', 'success');
          students = await BYD.call('getStudents', {});
          renderStudents(); renderHome();
        };
        if (goingActive) { action(); }
        else confirmAction('Remove ' + btn.dataset.studentToggleActive + ' from the roster? They will not be able to log in, but their homework/attendance/RFD history is kept. This can be undone from here.', 'Remove student', action);
      });
    });
    document.querySelectorAll('[data-hard-delete-student]').forEach(function (btn) {
      btn.addEventListener('click', function () {
        const email = btn.dataset.hardDeleteStudent;
        confirmHardDelete(email, 'student account', async function () {
          await BYD.call('deleteUserPermanently', { email: email });
          BYD.toast('Student permanently deleted.', 'success');
          students = await BYD.call('getStudents', {});
          renderStudents(); renderHome();
        });
      });
  ...
  async function loadSettings() {
    ...
    document.getElementById('scheduleSheetUrl').value = map.ScheduleSheetPublishedUrl || '';
    document.getElementById('roomCount').value = map.RoomCount || '15';
  }
    ...
    try {
      await BYD.call('updateSettings', {
        settings: {
          ...
          ScheduleSheetPublishedUrl: document.getElementById('scheduleSheetUrl').value.trim(),
          RoomCount: document.getElementById('roomCount').value.trim() || '15'
        }
      });
